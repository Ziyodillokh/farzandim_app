...
class TrackCleaner {
  ...
  static List<ChildLocation> clean(
    ...
    final kept = <ChildLocation>[accurate.first];
    for (var i = 1; i < accurate.length; i++) {
      if (_distM(kept.last, accurate[i]) >= minMoveM) kept.add(accurate[i]);
    }
    if (!identical(kept.last, accurate.last)) kept.add(accurate.last);
